...
return {
...

  {
    ...
    -- stylua: ignore
    keys = {
      { prefix .. "t", function() require("neotest").run.run(vim.fn.expand("%")) end, desc = "Run File (Neotest)" },
      { prefix .. "T", function() require("neotest").run.run(vim.uv.cwd()) end, desc = "Run All Test Files (Neotest)" },
      { prefix .. "r", function() require("neotest").run.run() end, desc = "Run Nearest (Neotest)" },
      { prefix .. "l", function() require("neotest").run.run_last() end, desc = "Run Last (Neotest)" },
      { prefix .. "s", function() require("neotest").summary.toggle() end, desc = "Toggle Summary (Neotest)" },
      { prefix .. "o", function() require("neotest").output.open({ enter = true, auto_close = true }) end, desc = "Show Output (Neotest)" },
      { prefix .. "O", function() require("neotest").output_panel.toggle() end, desc = "Toggle Output Panel (Neotest)" },
      { prefix .. "S", function() require("neotest").run.stop() end, desc = "Stop (Neotest)" },
      { prefix .. "w", function() require("neotest").watch.toggle(vim.fn.expand("%")) end, desc = "Toggle Watch (Neotest)" },
---@diagnostic disable-next-line: missing-fields
      { prefix .. "d", function() require("neotest").run.run({strategy = "dap"}) end, desc = "Debug Nearest" },
    },
    opts = {
